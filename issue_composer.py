...
def create_github_issue(repo, title, body, token):
    ...
    if response.status_code == 201:
        ...
        issue_link = response.json()["html_url"]
        return issue_link
    elif response.status_code == 429:
        retry_after = int(response.headers.get("Retry-After", 1))
        print(f"Rate limited. Retrying after {retry_after} seconds.")
        time.sleep(retry_after)
        issue_link = create_github_issue(repo, title, body, token)  # Retry the request
        ...
        print("Repository with restricted access.")
        return 422
    else:
        print("Failed to create issue. Error:", response.text, "Status code:", response.status_code)
    return issue_link


def get_smell_example_code(smell_name):
    with open(f"examples\\{smell_name}.txt") as f:
        return f.read()
...
def create_message(row, otherfiles):
    ...
    # get example
    smell_example = get_smell_example_code(smell_ref['smell_id'].values[0])
    ...
    other_file_message = ""
    # add examples of the paper
    message = message.replace("$smell_example$", smell_example)
    last_commit_id = get_last_commit_id(row['github_repo'].split("/")[0], row['github_repo'],
                                        code_link.replace("\\", "/"))
    print("Last commit id:", last_commit_id)
    message = message.replace("$master$",
                              get_last_commit_id(row['github_repo'].split("\\")[0], row['github_repo'].split("\\")[1],
                                                 code_link.replace("\\", "/")))
